...
<?php
    ...
    $stmt = $conn->prepare("UPDATE user SET ban_status = 1 WHERE id = ?");
    $stmt->bind_param("s", $user_id);   
    
    $user_id = trim(filter_var($_POST['user_id'], FILTER_SANITIZE_STRING, FILTER_FLAG_STRIP_HIGH));
    
    if($stmt->execute()){
        $_SESSION['proc_message'] = 'Action Successful, user banned.';
        header('Location:userList.php');
        exit();
    }
    else{
        $_SESSION['proc_error'] = "Action aborted, banning unsuccessful, please contact your administrator";
        header('Location:userList.php');
        exit(); 
    }
?>